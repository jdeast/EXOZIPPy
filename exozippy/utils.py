...
# ⚠️ Initial auto-translation from IDL (ChatGPT). Review required.
def parse_sed_file(sedfile, nstars):
    """
    Parse an SED file with optional blending and differential magnitude information.
    ...
    with open(sedfile, 'r') as f:
        for line in f:
            ...
            sedbands.append(parts[0])
            mags.append(float(parts[1]))
            errs.append(float(parts[2]))

            b = np.zeros(nstars, dtype=int)
            if len(parts) == 5:
                if '-' in parts[4]:
                    pos, neg = parts[4].split('-')
                    for i in map(int, pos.split(',')):
                        if 0 <= i < nstars:
                            b[i] = 1
                    for i in map(int, neg.split(',')):
                        if 0 <= i < nstars:
                            b[i] = -1
                else:
                    for i in map(int, parts[4].split(',')):
                        if 0 <= i < nstars:
                            b[i] = 1
            else:
                b[:] = 1  # default: all stars blended
            blend.append(b)

    sedbands = np.array(sedbands)
    mags = np.array(mags)
    errs = np.array(errs)
    ...
    if np.count_nonzero(good) <= 1:
        raise ValueError("At least two bands must have errors < 1 mag")

    return sedbands[good], mags[good], errs[good], blend[good]
