        ...
        for i in range(nstars):
            ...
            star = {"rootlabel":"Stellar Parameters:",
                    "radius": Parameter("rstar_" + str(i), lower=1e-1, upper=2000, initval=1.0, unit=u.solRad,
                                        latex='R_*', description='Radius', latex_unit="\\rsun", user_params=user_params),
                    ...
                    "teff": Parameter("teff_" + str(i), lower=1.0, upper=5e5, initval=5778, unit=u.K,
                                      latex='T_{\\rm eff}', description="Effective Temperature", latex_unit='K', user_params=user_params),
            ...
            if parsec[i] or mist[i]:
                ...
                star["initfeh"] = Parameter("initfeh_" + str(i), lower=-5.0, upper=5.0, initval=0.0,
                                            latex='[{\\rm Fe/H}]_{0}', description='Initial Metallicity', latex_unit='dex', user_params=user_params)
            ...
            if sedfile != None or mannrad[i] or mannmass[i]:
                star["distance"] = Parameter("distance_" + str(i), lower=1e-6, upper=1.426e10, initval=10.0,
                                             unit=u.pc, latex='d',description="Distance",latex_unit='pc', user_params=user_params)
                ...
                if mannmass[i] or mannrad[i]:
                    ...
                    star["absks"] = Parameter("absks_"+str(i),
                                              expression= star["appks"].value - 2.5*pt.log10((star["distance"].value/10.0)**2),
                                              unit=u.mag,latex='Ks',description="Absolute Ks mag",latex_unit='mag', user_params=user_params)
...
#                                                     ((star["radius"].value - mann_rstar) / mann_sigma_rstar) ** 2)
            ...
            if sedfile != None:
                star["av"] = Parameter("av_" + str(i), lower=0.0, upper=100.0, initval=0.0,
                                       unit=u.mag,
                                       latex='A_V', description='V-band extinction', latex_unit='mag', user_params=user_params)
                ...
                star["rstarsed"] = Parameter("rstarsed_" + str(i), lower=1e-9, upper=2000.0, initval=1.0,
                                             unit=u.solRad, latex='R_{*,SED}',latex_unit='\\rsun', user_params=user_params)
                ...
                fbolsed = Parameter("fbolsed_" + str(i), 
                                    expression=star["lstar"].value / (4.0 * math.pi * (star["distance"].value * pc) ** 2.0),
                                    unit=u.erg/u.second/u.cm**2,latex_unit='erg~s$^{-1}$~cm$^{-2}$',
...
#                    (star["fbol"].value - fbolsed) / (star["fbol"].value * fbolsedfloor)) ** 2)
        ...
        for i in range(nplanets):
            ...
            if fitlogmp:
                ...
                planet["logmp"] = Parameter("logmp" + str(i), lower=-10.0, upper=10.0, initval=0.0, unit=u.dex(u.jupiterMass),
                                            latex='\log{M_P}',description='Log of mp',latex_unit='', user_params=user_params)
...
#                massbound = pm.Potential("mp_bound_" + str(i), pt.switch(total_mass > 0.0, 0.0, -np.inf))
            ...
            planet["arsun"] = Parameter("arsun_" + str(i),
                                        expression=pt.power((G*(event["star"][starndx]["mass"].value +
                                                                planet["mp"].value * mjup) * planet["period"].value ** 2 / 
                                                             (4.0 * math.pi ** 2)),1.0 / 3.0),
                                        latex="a/R_{\sun}",description="Semi-major axis in \rsun", latex_unit='\\rsun', user_params=user_params)

            planet["ar"] = Parameter("ar_" + str(i),
                                     expression=planet["arsun"].value / event["star"][starndx]["radius"].value,
                                     latex='a/R_*',description='Semi-major axis in $R_*$',latex_unit='', user_params=user_params)
...
#                wbound = pm.Potential("wbound_" + str(i), pt.switch(L > 1.0,-np.inf,0.0))
                ...
                if not nochord[i]:
                    ...
                    planet["cosi"] = Parameter("cosi_" + str(i),
                                               ...
                                               (planet["ar"].value * (1.0 - planet["e"].value ** 2) / (1.0 + planet["esinw"].value)),
                                               latex='\cos{i}',description='cos of inclination',
                                               latex_unit='',user_params=user_params)

                    jacobian *= planet["b"].value ** 2 / (planet["cosi"].value * planet["chord"].value) # d(chord)/d(cosi)
                else:
                    planet["cosi"] = Parameter("cosi_" + str(i), lower=0.0, upper=1.0,
                                               latex='\cos{i}',description='cos of inclination',
                                               latex_unit='',user_params=user_params)
                    planet["b"] = Parameter("b_" + str(i),
                                            expression=planet["ar"].value*planet["cosi"].value*(1.0-planet["e"].value**2)/(1.0 - planet["esinw"].value),
                                            latex='b',description='impact parameter',
                                            latex_unit='',user_params=user_params)
...
#            ebound = pm.Potential("ebound_" + str(i), pt.switch(planet["e"].value < maxe, 0.0, -np.inf))
            ...
            planet["k"] = Parameter("k_" + str(i),
                                    expression=(2.0*math.pi*G/(planet["period"].value*(event["star"][starndx]["mass"].value+planet["mp"].value * mjup)**2.0))**(1.0/3.0) * 
                                    planet["mp"].value * mjup * pt.sin(planet["inc"].value)/pt.sqrt(1.0-planet["e"].value**2.0)*
                                    rsun/meter/86400.0,
