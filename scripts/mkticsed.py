...
def strom_conv(V, sigV, by, sigby, m1, sigm1, c1, sigc1):
    """Convert Strömgren catalog indices to individual uvby magnitudes."""
...
def mkticsed(ticid, star_name="Host", outpath=".", priorfile=None, sedfile=None,
    ...
    if not gaia_dr3_done and np.isfinite(dr2_fallback_plx):
        notes.append(f"DR3 parallax unavailable; using Gaia DR2 with Lindegren+2018 correction")
    ...
    if q2m is not None and len(q2m) > 0:
        ...
        if m_row == -1:
            ...
            if sep < 2.0:
                m_row = idx
        if m_row >= 0:
            jmag = _get(q2m, 'Jmag', m_row);  ejmag = _get(q2m, 'e_Jmag', m_row)
            hmag = _get(q2m, 'Hmag', m_row);  ehmag = _get(q2m, 'e_Hmag', m_row)
            kmag = _get(q2m, 'Kmag', m_row);  ekmag = _get(q2m, 'e_Kmag', m_row)
    ...
    if qw is not None and len(qw) > 0:
        ...
        if w_row == -1:
            ...
            if sep < 15.0:
                w_row = idx
        if w_row >= 0:
            w1 = _get(qw, 'W1mag', w_row);  ew1 = _get(qw, 'e_W1mag', w_row)
            w2 = _get(qw, 'W2mag', w_row);  ew2 = _get(qw, 'e_W2mag', w_row)
            w3 = _get(qw, 'W3mag', w_row);  ew3 = _get(qw, 'e_W3mag', w_row)
            w4 = _get(qw, 'W4mag', w_row);  ew4 = _get(qw, 'e_W4mag', w_row)
    ...
    if max_av is not None and max_av > 0:
        ...
        notes.append(f"Av < {max_av:.4f} mag  (3.1 × E(B-V) Schlegel+1998 upper limit)")
    ...
    # ── 7. [Fe/H] from Paunzen+2015 Strömgren if TIC has none ───────────────
    if not np.isfinite(feh_tic) and key('feh') not in yaml_data:
        print("Querying Paunzen+2015 Strömgren for [Fe/H] …", flush=True)
            ...
            if p_row >= 0:
                ...
                if np.isfinite(by) and np.isfinite(m1) and np.isfinite(c1) and m1 > 0:
                    ...
                    if 0.23 < by < 0.63 and 0.05 < m1 <= 0.68 and 0.13 < c1 <= 0.60:
                        ...
                        notes.append("[Fe/H] from Paunzen+2015 Strömgren via Cassegrande+2011 eq. 2")
                    # Cassegrande+2011, eq. 3 (cooler range)
                        ...
                        notes.append("[Fe/H] from Paunzen+2015 Strömgren via Cassegrande+2011 eq. 3")
    ...
    if qtyc2 is not None and len(qtyc2) > 0:
        ...
        bt = _get(qtyc2, 'BTmag', t_row);  ebt = _get(qtyc2, 'e_BTmag', t_row)
        vt = _get(qtyc2, 'VTmag', t_row);  evt = _get(qtyc2, 'e_VTmag', t_row)
    ...
    if qucac is not None and len(qucac) > 0:
        ...
        B_mag = _get(qucac, 'Bmag', u_row);  eB = _get(qucac, 'e_Bmag', u_row)
        V_mag = _get(qucac, 'Vmag', u_row);  eV = _get(qucac, 'e_Vmag', u_row)
        g_mag = _get(qucac, 'gmag', u_row);  eg = _get(qucac, 'e_gmag', u_row)
        r_mag = _get(qucac, 'rmag', u_row);  er = _get(qucac, 'e_rmag', u_row)
        i_mag = _get(qucac, 'imag', u_row);  ei = _get(qucac, 'e_imag', u_row)
    ...
    # ── 10. Optional: Paunzen+2015 Strömgren photometry for SED ─────────────
    ...
    if qpz_full is not None and len(qpz_full) > 0:
        ...
        vmag = _get(qpz_full, 'Vmag',  pz_row); evmag = _get(qpz_full, 'e_Vmag', pz_row)
        by   = _get(qpz_full, 'b-y',   pz_row); eby   = _get(qpz_full, 'e_b-y',  pz_row)
        m1   = _get(qpz_full, 'm1',    pz_row); em1   = _get(qpz_full, 'e_m1',   pz_row)
        c1   = _get(qpz_full, 'c1',    pz_row); ec1   = _get(qpz_full, 'e_c1',   pz_row)
    ...
    if qmerm is not None and len(qmerm) > 0:
        ...
        V_m  = _get(qmerm, 'Vmag', me_row);  eV_m = _get(qmerm, 'e_Vmag', me_row)
        BV_m = _get(qmerm, 'B-V',  me_row);  eBV  = _get(qmerm, 'e_B-V',  me_row)
        UB_m = _get(qmerm, 'U-B',  me_row);  eUB  = _get(qmerm, 'e_U-B',  me_row)
    ...
    if qgalex is not None and len(qgalex) > 0 and 'FUV' in qgalex.colnames:
        ...
        fuv = _get(qgalex, 'FUV', ga_row);  efuv = _get(qgalex, 'e_FUV', ga_row)
        nuv = _get(qgalex, 'NUV', ga_row);  enuv = _get(qgalex, 'e_NUV', ga_row)
...
def main():
    ...
    p.add_argument('--stromgren',  action='store_true', help='Uncomment Strömgren photometry')
