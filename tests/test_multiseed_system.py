...
def _base_user_params():
    ...
    share one baseline."""
    return {
        # t_0/u_0 are per-source; the parallax vector is event-level.
...
def _config():
    # Post-split microlensing topology: the event options live on
    # `mulensevent:`, and `lens:`/`source:` name one physical body each.
    return {
...
def test_get_raw_starts_skips_seed_whose_start_violates_a_bound(caplog):
    ...
    lies outside the hard bound [-18, 18] (mulensing defaults.yaml, `source:`
    section),
...
def test_get_raw_starts_single_start_when_no_seed_list():
    ...
    """
    user_params = _base_user_params()
    user_params["source.Source.u_0"] = {"initval": 0.3}
...
def test_dc2018_128_example_builds_with_list_valued_mmexofast_seeds(tmp_path):
    ...
    Given the real DC2018_128 example (config + params.yaml regenerated by
      scripts/mmexofast_to_params.py's all-solutions default:
      source.Source.t_0/u_0/rho, mulensevent.t_E and
      lens.Companion.s/alpha/q initvals are all 2-element lists, one
      MMEXOFAST solution each),
    ...
    Then it succeeds and solves 2 seeds. This is a regression test for
      ...
      checks, mulensevent.t0_par) that read raw user_params values directly and
