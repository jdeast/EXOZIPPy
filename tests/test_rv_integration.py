...
np.random.seed(42)
_omegas = np.concatenate(
    [np.linspace(0, 2 * np.pi, 9)[:-1], np.random.uniform(0, 2 * np.pi, 5)]
)
_eccs = [
    0.2 if i < 8 else float(np.random.uniform(0.1, 0.8))
...
@pytest.mark.parametrize(
    "omega_ecc", _CASES, ids=[_case_id(c) for c in _CASES]
)
def test_radial_velocity_handles_descending_node_and_eccentricity(
    omega_ecc, compiled_rv_functions
):
    """
    ...
    k_param = Parameter(
        label="K", unit="m/s", internal_unit="solRad/d", initval=_K_USER_MS
    )

    Tp_val = tp_fn(logP_in, tc_in, secosw_in, sesinw_in)
    f_D = -omega
    E_D = 2.0 * np.arctan2(
        np.sqrt(1.0 - ecc) * np.sin(f_D / 2),
...
def test_radial_velocity_matches_kelt4_periastron_benchmark():
    ...
    with model:
        t_var = pt.vector("t")
        K_var = pt.vector("K_int")
        rv_node = orbit.get_radial_velocity(
            t_var, K_var, pt.as_tensor_variable([0], dtype="int32")
        )
        rv_fn = pytensor.function(
            model.free_RVs + [t_var, K_var], rv_node, on_unused_input="ignore"
        )

        k_user_ms = 108.6
        k_int = k_user_ms * orbit.config_manager.get_conversion_factor(
            "planet", "K"
        )

        init_point = model.initial_point()
        p_vals = [
            np.zeros_like(np.atleast_1d(init_point[v.name])).astype("float64")
        ...
        rv_internal = rv_fn(
            *p_vals,
            np.array([tp_eval], dtype="float64"),
            np.array([k_int], dtype="float64"),
        )
...
def test_radial_velocity_computes_correctly_over_vectorized_time_array():
    ...
    with model:
        ...
        rv_fn = pytensor.function(
            model.free_RVs + [t_pt, K_pt, ndx_pt],
            rv_node,
            on_unused_input="ignore",
        )
