...
variations), building on fitthermal (PR 1.a).
  ...
  - Band/Planet manifest wiring: fitreflect/fitellip/fitbeam/
    derivebeam parsing and the opt-in pinning gate (mirrors
    fitthermal's).
...
def test_planet_fitbeam_and_derivebeam_together_derives():
    """
    Given both fitbeam and derivebeam set on the same planet,
    When register_parameters runs,
    Then derivebeam wins: beam is still derived from K via the
...
def test_beam_off_does_not_require_K_no_orbit_config():
    ...
    (calc_beam_from_K, for derivebeam), which is what exposed it.
