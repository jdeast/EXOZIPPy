...
def test_derived_prior_numerical_evaluation():
    ...
    with pm.Model() as model:
        logmass = Parameter(
            label="logmass", initval=0.0, init_scale=0.1, lower=-2.0, upper=2.0
        )
    ...
    np.testing.assert_allclose(
        ...
        -8.0,
        atol=1e-5,
        err_msg="Derived prior calculated incorrect penalty!",
    )
...
def test_sampled_parameter_fallback_scaling():
    ...
    with pm.Model() as model:
        logP = Parameter(
            label="logP", initval=0.5, init_scale=0.01, lower=-1.0, upper=2.0
        )
        logP.build_pymc()

        ecc = Parameter(
            label="ecc", initval=0.05, mu=0.0, sigma=0.05, lower=0.0, upper=1.0
        )
        ecc.build_pymc()
    ...
    logp_center = logp_fn(
        {"ecc_raw": np.array([0.0]), "logP_raw": np.array([0.0])}
    )
