...
def query_gost():
    ...
    for m in re.finditer(
        ...
        r"</eventTcbBarycentricJulianDateAtBarycentre>",
        xml,
        re.S,
    ):
    ...
    np.savetxt(
        ...
        transits,
        header=(
            "GOST forecast for Gaia BH1 (ra=262.17120816, "
...
def main():
    ...
    if max_diff > 0.01:
        raise SystemExit(
            "Parallax factor mismatch with GOST -- "
    ...
    np.savetxt(
        OUTFILE,
        np.column_stack([t, w, err, psi / DEG2RAD]),
        header=(
            "Simulated Gaia epoch astrometry for Gaia BH1 "
