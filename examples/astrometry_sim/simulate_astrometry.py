...
def main():
    ...
    np.savetxt(
        ...
        header=(
            "Simulated 2-D absolute astrometry "
    ...
    pa = np.degrees(np.arctan2(dE_r, dN_r)) % 360.0 + rng.normal(
        0, PA_ERR, len(t_rel)
    )
    np.savetxt(
        "SimBinary.Ground.rel.astrom",
        np.column_stack(
            [
                t_rel,
                sep,
                np.full_like(t_rel, SEP_ERR),
                pa,
                np.full_like(t_rel, PA_ERR),
            ]
        ),
        header=(
            "Simulated relative astrometry "
            "(see simulate_astrometry.py)\n"
