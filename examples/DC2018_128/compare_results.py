...
for sol_idx, sol in enumerate(mmx_data["fits"]):
    ...
    s = sol.get("sigmas", {})
    mmxf[sol_idx] = {
        k: p[k] for k in ("t_0", "u_0", "t_E", "rho", "s", "q", "alpha")
    }
    mmxs[sol_idx] = {
        "t_0": s.get("t_0"),
        "u_0": s.get("u_0"),
        "t_E": s.get("t_E"),
        "rho": (
            p["rho"] * s["log_rho"] * np.log(10) if "log_rho" in s else None
        ),
...
def _parse_csv(csv_path):
    ...
    with open(csv_path, newline="") as f:
        reader = csv.DictReader(
            f, fieldnames=["parname", "value", "up_err", "low_err"]
        )
        for row in reader:
...
print(
    "Note: alpha convention is unknown for DC18 truth values — no remapping applied."
)
print("      EXOZIPPy uses the center of mass as the origin for alpha.")
print(
    "      Direct comparison of alpha may be meaningless without convention alignment."
)
