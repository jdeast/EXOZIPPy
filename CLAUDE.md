...
`<prefix>_trace.nc` carries the `evaluator.structural_hash` of the config + params that produced it (root attrs `exozippy_structural_hash`, plus the payload it was hashed over so a mismatch can name what changed). All three reloads check it -- `run.py`'s `recompute_trace: false` branch, the `exozippy-modes` CLI, and `mkparam.mkprior` (whose restart file seeds the *next* fit, so a stale trace there corrupts a run that has not happened yet; `run.py` hands its live `System.structural_fingerprint()` to the automatic end-of-run call so that one can never be refused for a spurious reason). A mismatch **raises `StaleTraceError`**; there is no opt-out key. This is deliberately harsher than the neighboring whitening reload, which warns and re-measures: whitening can be honestly recomputed at load time, whereas foreign draws cannot be repaired, and reusing them silently regenerates every posterior, LaTeX table and plot under the new labels. A trace with **no** hash (written before this existed) is "unverifiable", not stale -- one warning, then it loads. The hash is snapshotted in `System.__init__` (`System.structural_fingerprint()`), before ConfigManager standardizes/strips the params and before stage 1-2 hints touch the config, so it never depends on lifecycle stage. Known blind spots -- shared with the GUI's stale/live decision, so do not widen `structural_hash` without considering that consumer: data-file **contents** (only paths are hashed), numeric `mu`/`sigma` magnitudes, and per-file component keys such as `gp:`, `likelihood:`, `mask:` or `mass_parameterization:`.
