...
- **Never compose branch log-densities with `pt.where`** (the JAX where-trap): `where`'s VJP multiplies the unselected branch by zero, and `0 * NaN` or `0 * inf` poisons the gradient of the whole expression on every backend, while the C backend's finite dlogp hides it. Combine branch logps with `pt.logaddexp`, keep a floor on a radicand rather than on the result -- **and that floor must be STRICTLY POSITIVE**, because `pt.maximum(x, 0.0)` has zero gradient on the clamped side while `sqrt'(0)` is infinite, so flooring a radicand *at zero* rebuilds the very `0 * inf` this rule exists to prevent (`ECC_FLOOR`/`CHORD_RADICAND_FLOOR` in `components/orbit/physics.py` are the two worked examples; the second cost 46% of a shipped parameterization's support and froze chains at the denormal step size for a whole run, reviews 1.8.2 and 1.8.5) -- and use the soft-bound helpers in `src/exozippy/potentials.py` for a penalty that needs a gradient pointing back. A soft bound cannot rescue a NaN elsewhere in the sum: one NaN poisons the whole gradient vector, so the hard shield being finite is a *precondition* for the soft one doing anything. Likewise assemble a mixed value vector with `pt.set_subtensor`, not `pt.where`.
...
- Before touching how data files are read (columns, masks), detrending, the noise model, Gaussian-process noise or robust likelihoods, read `src/exozippy/components/instrument.md`. The astronomical time systems (`time_frame`, `time_scale`, BJD_TDB conversion) are a separable mixin in `src/exozippy/components/timesystem.py`, deliberately NOT part of the field-neutral data scaffolding -- its defaults convert nothing, so a component from another field could inherit it, work, and still advertise a barycentric correction it never applies.
...
- Before changing the credible-interval width, the table caption, or anything that summarizes a posterior, read the "credible-interval width" section of `src/exozippy/outputs/outputs.md`. The width is ONE run-level setting (`src/exozippy/reporting.py`, `reporting: {credible_interval: ...}`), defaulting to the 1-sigma astronomy convention; median + 68% is not universal, and a component reporting for another field reports in that field's convention.
- Before touching the pharmacokinetics components, read `src/exozippy/components/pharmacokinetics/pharmacokinetics.md` -- and its `README.md` first, which states plainly that the set was written by an astronomer and an LLM with no domain reviewer and is a demonstration of the component-agnostic architecture, not a validated PK tool. That doc also records what building a non-astronomy component found about the core.
