...
Measured, so nobody has to re-derive it: on `examples/DC2018_128`, the whitened curvature block of (`star.logmass_raw`, planet mass coordinate) at the start improves in condition number from ~48 (linear) to ~11 (log_q), stable across finite-difference step sizes. Real, but a 4.5x on one 2x2 block -- **not** the order-of-magnitude the "wasted dynamic range" framing suggests, and the residual correlation does not vanish (+0.07 -> -0.10). Do not sell this as the headline. The unambiguous wins are structural: negative `q` becomes unreachable rather than likelihood-flat, and the Chen plateau disappears.
