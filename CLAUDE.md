...
- `Instrument.outlier_prob_at_data(system, point)` returns per-point posterior outlier probabilities (`(n_total_obs,)`, zero for non-hogg files; lazily compiled against `system.plot_params`, evaluate per posterior draw) -- the auditable replacement for a hard bad-data mask. `examples/DC2018` turns `likelihood: hogg` on for every light curve; the MMEXOFAST `excluded_points` mask still applies first, the mixture catches what survives it.
