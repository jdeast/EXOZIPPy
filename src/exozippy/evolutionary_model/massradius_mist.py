...
def massradius_mist(
    ...
    vvcritndx = (
        0 if vvcrit is None else np.where(allowedvvcrit == vvcrit)[0][0]
    )
    ...
    if eep >= neep:
        if verbose:
            logger.warning(
                f"EEP ({eep}) is out of bounds for track with {neep} points"
            )
        return np.inf

    # Interpolation using two closest ages
    x_eep = (
        (age - ages[eep - 1]) / (ages[eep] - ages[eep - 1])
        if ages[eep] != ages[eep - 1]
        else 0.0
    )
    ...
    ageweight_interp = (1 - x_eep) * ageweights_data[
        eep - 1
    ] + x_eep * ageweights_data[eep]
    ...
    chi2_rstar = (
        (mistrstar - rstar)
        / (
            rstarfloor * mistrstar
            if rstarfloor > 0
            else percenterror * mistrstar
        )
    ) ** 2
    chi2_teff = (
        (mistteff - teff)
        / (tefffloor * mistteff if tefffloor > 0 else percenterror * mistteff)
    ) ** 2
    chi2_feh = (
        (mistfeh - feh) / (fehfloor if fehfloor > 0 else percenterror)
    ) ** 2
    chi2_age = (
        (mistage - age)
        / (agefloor * mistage if agefloor > 0 else percenterror * mistage)
    ) ** 2
