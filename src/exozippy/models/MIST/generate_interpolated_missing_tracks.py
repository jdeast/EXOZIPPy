...
def _load_single_track(feh_code, alpha_code, vvcrit, mass, processed_path):
    fname = (
        processed_path
        / f"feh_{feh_code}_afe_{alpha_code}_vvcrit{vvcrit:0.1f}.parquet"
    )
    df = pd.read_parquet(fname)
    track = (
        df[np.isclose(df["mass"], mass)]
        .sort_values("EEP")
...
def _generate_interpolated_track(row, alpha, vvcrit, processed_path):
    ...
    lo_val, hi_val = sorted(
        [
            _parse_neighbor_value(row["neighbor_1"]),
    ...
    if method == "alpha":
        ...
        target = alpha
        lo_track = _load_single_track(
            feh_code, ALPHA_CODES[str(lo_val)], vvcrit, mass, processed_path
        )
        hi_track = _load_single_track(
            feh_code, ALPHA_CODES[str(hi_val)], vvcrit, mass, processed_path
        )
    elif method == "initfeh":
        alpha_code = ALPHA_CODES[str(alpha)]
        target = initfeh
        lo_track = _load_single_track(
            _feh_value_to_code(lo_val),
            alpha_code,
            vvcrit,
            mass,
            processed_path,
        ...
        target = mass
        lo_track = _load_single_track(
            feh_code, alpha_code, vvcrit, lo_val, processed_path
        )
        hi_track = _load_single_track(
            feh_code, alpha_code, vvcrit, hi_val, processed_path
        )
    else:
    ...
    if not np.array_equal(lo_track["EEP"].values, hi_track["EEP"].values):
        raise ValueError(
            f"EEP index mismatch between neighbor tracks for initfeh={initfeh}, mass={mass}"
        )
    ...
    for col in BLENDED_COLUMNS:
        new_track[col] = (1 - weight_hi) * lo_track[
            col
        ].values + weight_hi * hi_track[col].values

    new_track["here_be_dragons"] = np.maximum(
        lo_track["here_be_dragons"].values, hi_track["here_be_dragons"].values
    )
    new_track["dEEP_dage"] = np.gradient(
        new_track["EEP"].values, new_track["age_mist"].values
    )
...
def generate_missing_tracks_for_alpha_vvcrit(
    alpha,
    vvcrit=0.0,
    ...
    if len(interpolatable_rows) == 0:
        print(
            f"No interpolatable points found for alpha={alpha}, vvcrit={vvcrit}."
        )
        return {}

    tracks_by_file = {}
    for _, row in interpolatable_rows.iterrows():
        track = _generate_interpolated_track(
            row, alpha, vvcrit, processed_path
        )
        ...
        print(
            f"Generated interpolated track: initfeh={row['initfeh']}, mass={row['mass']} "
...
if __name__ == "__main__":
    generate_missing_tracks_for_alpha_vvcrit(
        alpha=0.0,
        vvcrit=0.0,
