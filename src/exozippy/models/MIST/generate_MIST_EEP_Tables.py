...
from exozippy.models.MIST.parse_MIST_EEP_filenames import _generate_MIST_EEP_url, _parse_initfeh_alpha_vvcrit_from_name
...
EEP_RAW_TRACKS_PATH_DEFAULT = Path("/Volumes/Data/EEP_Tracks/MISTv2.5/raw_tracks/")
...
def _add_or_grab_solar_values_to_MIST_yaml(EEP_path = EEP_RAW_TRACKS_PATH_DEFAULT, 
...
def _generate_alpha_vvcrit_filename_parts(alpha: float, vvcrit: float, 
                                          version: Literal["1.2", "2.5"] = "2.5"):

    # alpha part
    filename_alpha_part = "afe_"
    filename_alpha_part += "m" if alpha < 0 else "p"
    filename_alpha_part += f"{abs(alpha)*10:.0f}" if version == "2.5" else f"{abs(alpha):0.1f}"
...
def generate_EEP_grid_for_alpha_vvcrit(alpha, vvcrit, 
    ...
    for file in filepaths:
        _, alpha_val, vvcrit_val = _parse_initfeh_alpha_vvcrit_from_name(file.name)
        alpha_val = np.round(alpha_val, decimals=1)
        if vvcrit_val == vvcrit and alpha_val == alpha:
            alpha_vvcrit_files.append(file)

    frames = [pd.read_parquet(file, engine="pyarrow") for file in alpha_vvcrit_files]

    if len(frames) == 0:
        print(f"No files found in {processed_path.name} for alpha={alpha}, vvcrit={vvcrit}")
        return
    
    alpha_vvcrit_df = pd.concat(frames, ignore_index=True)

    filename_alpha_part, filename_vvcrit_part = _generate_alpha_vvcrit_filename_parts(alpha, vvcrit, version)
...
def __main_step1_process_raw_eep_tracks__():
    ...
    for folderpath in EEP_RAW_PATH.iterdir():
        ...
        if folderpath.is_dir():
            ...
            process_eep_directory(folderpath, save=True, output_path=EEP_PROCESSED_PATH)
...
def __main_step2_generate_eep_grids__():
    ...
    for alpha in alpha_grid_values:
        for vvcrit in vvcrit_values:
            df = generate_EEP_grid_for_alpha_vvcrit(alpha, vvcrit, 
                                                    processed_path=EEP_PROCESSED_PATH, 
                                                    grid_path=EEP_GRID_PATH, save=True)
