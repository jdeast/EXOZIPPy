...
class Contour:
    ...
    def _generate_contours(self):
        ...
        xmin, xmax = self.x.min() - 3*self.x_err, self.x.max() + 3*self.x_err
        ymin, ymax = self.y.min() - 3*self.y_err, self.y.max() + 3*self.y_err
...
def plot_contours(groups, xlabel=r"$T_{\rm{eff}}$ (K)", ylabel=r"$\log g$ (cgs)", figsize=(7, 6)):
    ...
    for g in groups:
        if isinstance(g, Contour):
            ...
            ax.scatter(g.x, g.y, s=size, color=to_rgba(color, alpha), label=label, zorder=3)

            contour_alphas = [0.25, 0.5, 0.75]  # Different alpha values for different confidence levels
            rgba_colors_contours = [to_rgba(color, alpha=a) for a in contour_alphas]

            cf = ax.contourf(g.X, g.Y, g.Z, levels=g.levels + [g.Z.max()], colors=rgba_colors_contours)
            cl = ax.contour(g.X, g.Y, g.Z, levels=g.levels, colors=rgba_colors_contours, linewidths=linewidth)
