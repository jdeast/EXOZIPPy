...
- `symbolic_physics.py` — SymPy `RELATIONS` (equations) and `get_symbol_map(component_config, system_config)` (maps abstract symbols → indexed YAML paths); must set `comp_key = "<yaml_key>"` to match the YAML block name. **Both arguments are required**: the second is the WHOLE parsed config, because a map builder may need another component's instances — the `mulensevent` split moves the lens and source body lists off the component whose relations reference them. There is no one-argument spelling and no signature introspection; an internal interface carries exactly one form.
