...
% In step through C28; C29 (band-extinction convention) and C30 (the
% astrometric centroid shift) are in this repo's pair only and OWED to the
% paper.  It cannot ride the same commit, so
...
crowded bulge field it can dominate.  We treat the point-source, single-lens
case only: averaged over a resolved source disk the shift nearly vanishes at
$\rho \sim u$ and reverses sign beyond, so the expression is valid where
$\rho \ll u$ --- conveniently the complement of the finite-source events
that already measure $\theta_{\rm E}$ through $\rho$.
