...
% developer-facing list: it carries the SAME numbered claims C1-C30, and
...
%
% A THIRD COPY LIVES OUTSIDE THIS REPOSITORY: the Conventions section of
% paper3_microlensing.tex in the paper repo (~/old_home/papers/exozippy,
% remote on Overleaf) carries the same C-numbers as the submitted text.
% In step through C30 (paper commits 3476111, 2026-09-23, added C29 and
% C30 there; 7eeeb42, 2026-09-24, brought C30 to stage 2).  It cannot ride
% the same commit, so
% the rule is that a C-rule is not finished until all three carry it:
% pull the paper repo, make the matching edit there, and name that commit
% in this one.  The paper is not licensed to run ahead.
% =====================================================================
...
\paragraph{C30 --- the astrometric centroid shift is referenced to the
source's unlensed position and points away from the lens.}  A point lens
splits an unresolved source into two images on the lens--source axis
\citep{Walker:1995, Hog:1995, Miyamoto:1995}.  Their flux-weighted centroid,
measured from the source's \emph{unlensed} position --- the position an
astrometric time series references through the source's coordinates, proper
motion and parallax --- is
\begin{equation}
  \delta\boldsymbol{\theta}(t) = -\,\theta_{\rm E}\,
  \frac{\Delta\boldsymbol{\theta}(t)}{|\mathbf{u}(t)|^2 + 2},
\end{equation}
where $\Delta\boldsymbol{\theta}$ is the lens-minus-source separation of C9
in units of $\theta_{\rm E}$.  The shift therefore points along the
lens$\rightarrow$source axis, away from the lens, with magnitude
$\theta_{\rm E}\,u/(u^2+2)$, peaking at $0.354\,\theta_{\rm E}$ at
$u = \sqrt{2}$ and decaying only as $\theta_{\rm E}/u$; over an event the
apparent position traces an ellipse whose size is set by $\theta_{\rm E}$
\citep{Walker:1995, Hog:1995}.  It has support for years on either side of
a $t_{\rm E}$-long photometric event and so belongs inside the full
astrometric model rather than in an event-window correction; $\theta_{\rm
E}$ enters as a linear amplitude on a shape the light curve already
determines, which is what makes it a direct measurement of the Einstein
radius and hence of the lens mass \citep{Miyamoto:1995, Dominik:2000}.
Measured from the \emph{lens} the same
centroid sits at $\theta_{\rm E}\,u\,(u^2+3)/(u^2+2)$, which is the quantity
\texttt{VBMicrolensing} reports; subtracting the source position $u$
recovers the expression above identically.

What an instrument centroids is the sum of the images and the blend, so the
modelled offset from the source's unlensed track $\mathbf{x}_s(t)$ is
\begin{equation}
  \delta\mathbf{x}_{\rm obs}(t) = \frac{A f_s}{A f_s + f_b}\,
  \delta\boldsymbol{\theta}(t) + \frac{f_b}{A f_s + f_b}\,
  \bigl[\mathbf{x}_b - \mathbf{x}_s(t)\bigr],
\end{equation}
with $f_s$ and $f_b$ the source and blend fluxes in the astrometric band and
$\mathbf{x}_b$ the blend's photocenter, held fixed on the sky.  The first
term is a time-dependent dilution, suppressed exactly at peak; the second is
the blend dragging the centroid as the source brightens and fades, and in a
crowded bulge field it can dominate.  The closed form above is the
point-source, single-lens case; averaged over a resolved source disk the
shift nearly vanishes at $\rho \sim u$ and reverses sign beyond, so it is
valid where $\rho \ll u$.  For a finite source, a binary or an $N$-body
lens we take the centroid from the same contour integration that computes
the magnification \citep{Bozza:2018, Bozza:2025}: the light centroid of all
the images, integrated over the limb-darkened source disk, minus the source
position, both in the frame whose origin is the lens centre of mass (C12).
That origin is not stated in the code's documentation; we verified it
against a direct solution of the lens equation, which reproduces both the
magnification and the centroid to $10^{-8}$ under that origin and under no
other.
...
$s_j(t) = |\vec{\delta}_j(t)|$ and $\alpha_j(t) = \phi_\pi -
\mathrm{PA_{axis}}(t)$, where the minus sign is
Equation~\ref{eq:gammaperp} again.  That expression is absolute --- no
anchor enters, because C15's $\alpha$ simply \emph{is} $\phi_\pi$ minus
the axis's position angle, evaluated at every epoch rather than once ---
and it leaves the mode with \emph{no free geometry parameters whatever}:
neither $s_0$ nor $\alpha_0$ is sampled, and the geometry entering the
magnification is a function purely of the referenced orbit's elements
together with the $\phi_\pi$, $\theta_{\rm E}$ and $D_{\rm L}$ the fit
already carries.  That is the over-constraint \citet{Skowron:2011}
proposed.  The sign's correctness is pinned
...
\paragraph{C25 --- source orbital motion (xallarap).}  The parallax is the
\emph{observer's} offset; xallarap is the \emph{source's} own, and it
enters the trajectory at exactly the same slot with the same sign
discipline.  With $\delta\vec{\sigma}(t)$ the luminous source's sky offset
from its own barycentric motion, in Einstein radii and anchored at
$t_{0,\rm par}$ (the offset vanishes there, so $t_0$ and $u_0$ keep their
geocentric-frame meaning; one fiducial epoch anchors the parallax and both
orbital-motion effects),
\begin{equation}
\Delta\tau = -\,\delta\vec{\sigma}\cdot\hat{\tau}, \qquad
\Delta\beta = -\,\delta\vec{\sigma}\cdot\hat{\beta},
\end{equation}
where the minus sign is C7 (the trajectory is lens \emph{minus} source)
and $(\hat{\tau}, \hat{\beta})$ is C9's basis --- the pair the parallax
terms already project onto.  The offset is the source's Keplerian
barycentric track, $a_1/(D_{\rm S}\,\theta_{\rm E})$ in amplitude, so the
mode introduces \emph{no new fitted parameters}: the orbit's period,
eccentricity, orientation, and the companion mass (through the barycentric
scale) are the coordinates, i.e. the physical-parameter approach of
\citet{Mroz:2026} expressed through the model's component graph.  A linear
source drift is deliberately not offered: it is exactly degenerate with
$(t_{\rm E}, t_0, u_0, \alpha)$ in the light curve alone, and becomes
meaningful only where an external dataset constrains the source's proper
motion over a baseline much longer than $t_{\rm E}$.  A xallarap orbit
measures $\Omega$ (the track's orientation) but, unlike the lens case
(C24), remains node-degenerate: a sky track of any kind is invariant under
the sky-plane reflection.  The mapping onto the $\xi$ elements of
\citet{Zhai:2024} (the parameterization of \citealt{Mroz:2026}), verified
numerically against \texttt{MulensModel}'s implementation at the level of
the composed light curve, is
$\Omega = \phi_\pi + \xi_\Omega + 180^\circ$,
$i = \xi_i$, $\omega_* = \xi_\omega$,
$\xi_a = a_1/(D_{\rm S}\,\theta_{\rm E})$, with the phase carried by
$\nu(t_{0,\xi}) = \xi_u - \xi_\omega$.

\paragraph{C26 --- the Keplerian elements, and why $\alpha$ is not an
independent convention.}  The element set is the textbook one, expressed in
the frame of C1: $i$ is measured from the plane of the sky, $\Omega$ is the
position angle of the ascending node East of North (C2), and $\omega$ is
$\omega_*$, the argument of periastron of the \emph{primary's} orbit about
the system barycentre (C27).  The ascending node is the node at which the
body \emph{recedes} from the observer, which is what ties $\Omega$ to the
sign of the radial velocity and is the only reason the node is identifiable
at all (C28).  The sky position follows from the standard Euler application
$R_z(\Omega)\,R_x(i)\,R_z(\omega)$, so that at $\omega + f = 0$ the body
sits at position angle exactly $\Omega$; this is the same projection, and
the same single implementation, that the astrometric and radial-velocity
models share.

The reconciliation with microlensing is then one equation.  Writing
$\vec{\delta}_j(t)$ for companion $j$'s offset from the primary --- the
\emph{relative} orbit, hence $\omega_* + 180^\circ$ (C27) --- projected
through that Euler chain and scaled into Einstein radii by
$a/(D_{\rm L}\,\theta_{\rm E})$,
\begin{eqnarray}
  s_j(t)  &=& |\vec{\delta}_j(t)|, \qquad
  \mathrm{PA_{axis}}(t) \;=\; \mathrm{atan2}(\delta_{j,E}, \delta_{j,N}),
  \nonumber \\
  \alpha_j(t) &=& \phi_\pi - \mathrm{PA_{axis}}(t),
  \label{eq:alphakep}
\end{eqnarray}
with $\phi_\pi$ from Equation~(\ref{eq:phipi}).  Equation~(\ref{eq:alphakep})
is C15 and C20 evaluated at every epoch rather than once; its time
derivative is Equation~(\ref{eq:gammaperp}), so the linear and Keplerian
modes agree by construction rather than by a fitted offset.  Two
consequences are worth stating separately.

First, \emph{$\alpha$ is a derived quantity and not a convention}.  It is a
position angle subtracted from another position angle.  A microlensing
$\alpha$ and a Keplerian $\Omega$ can therefore never genuinely disagree
about orientation; a mismatch between them is always a disagreement about
where $\phi_\pi$ points (C8, hence about the frame of C5) or about which
end of the binary axis a position angle names (C20) --- both of which are
answerable without reference to either code's internals.  This is the
sense in which microlensing, which appears at first to carry an orientation
convention all its own, in fact carries none.

Second, in the Keplerian mode there is no free $\alpha_0$ and no free $s_0$
at all: both sides of Equation~(\ref{eq:alphakep}) are consumed, and the
binary geometry entering the magnification is a function purely of
$(P, t_p, e, \omega_*, i, \Omega, a)$ together with the frame quantities
$\phi_\pi$, $\theta_{\rm E}$ and $D_{\rm L}$ that the fit already carries.
The mode adds no parameters, which is precisely the over-constraint
\citet{Skowron:2011} proposed.  OGLE-2009-BLG-020 is our worked example and
the reason the machinery exists in this form: the Keck and Magellan radial
velocities of the lens primary \citep{Yee:2016} pin $P$, $t_p$, $e$,
$\omega_*$ and $K$, the light curve pins the projected geometry, and
$\Omega$ and $\mathrm{sign}(\cos i)$ --- which no radial velocity can see
--- become measured quantities rather than assumptions.  The published
$s_0 = 0.4294$ and $\alpha_0 = 189.08^\circ$ are an acceptance check on
that fit, not inputs to it.

\paragraph{C27 --- which body's $\omega$.}  A single orbit has two
arguments of periastron differing by exactly $180^\circ$: $\omega_*$, of
the primary's orbit about the barycentre, and $\omega_{\rm comp} = \omega_*
+ 180^\circ$, of the companion's orbit relative to the primary.  Neither
spelling is wrong and both are in common use --- the radial-velocity
literature reports $\omega_*$ because that is the orbit the spectrograph
measures, while the visual-binary literature reports the relative orbit
because that is the one the eye sees --- so an $\omega$ quoted without
naming its body is a $180^\circ$ ambiguity and nothing more.  We report and
sample $\omega_*$ throughout, and apply the flip internally wherever the
relative orbit is the one required, as in Equation~(\ref{eq:alphakep}).

The ambiguity is cheap to resolve whenever any radial information exists,
because a $180^\circ$ shift in $\omega_*$ inverts the velocity curve, and
this is worth doing rather than assuming.  \citet{Yee:2016} tabulate
$\omega_{\rm peri} = 151.600^\circ$ for OGLE-2009-BLG-020L.  Taken as
$\omega_*$ that value puts the radial-velocity model in antiphase with the
measurements, $\chi^2 = 2087$ for 14 points; at $\omega_* = 331.600^\circ =
151.600^\circ + 180^\circ$ the same model gives $\chi^2 = 3.2$.  The
published number is the companion's, as the visual-binary habit would
suggest, and the flip is a statement about vocabulary that the data confirm
in one evaluation.  Note also that xallarap (C25) inverts the usual
expectation: the orbit that enters a xallarap fit is the \emph{source's
own} track about its barycentre, so it is $\omega_*$ that maps across with
no flip, while the node acquires one.

\paragraph{C28 --- the node degeneracy, what breaks it, and the third-axis
trap.}  The transformation
\begin{equation}
  (\Omega,\, \omega_*) \;\longrightarrow\;
  (\Omega + 180^\circ,\, \omega_* + 180^\circ),
  \label{eq:nodefold}
\end{equation}
with the matching shift of $t_C$ and with $i$ \emph{unchanged}, is a
reflection through the plane of the sky: substituted into the Euler chain of
C26 it leaves the sky coordinates $(X, Y)$ identically unaltered and
reverses only $Z$.  It is therefore invisible to astrometry of every kind
--- absolute, relative, and the xallarap track of C25 --- and only radial
information identifies which node is the ascending one.  We declare it per
orbit rather than per system, because an orbit that a radial velocity
constrains is not degenerate at all even when it sits in a system full of
orbits that are, and we fold the two labels together when assessing
convergence so that a sampler exploring both is not mistaken for one that
has failed to converge.

For a lens binary there is a second thing that breaks it, and it is the one
genuinely microlensing-specific measurement in this section: the
\emph{rotation sense} of the binary axis on the sky.  Once the node is
fixed, that sense measures $\mathrm{sign}(\cos i)$, through the minus sign
of Equation~(\ref{eq:gammaperp}) --- equivalently, $\gamma_\perp
\rightarrow -\gamma_\perp$ is $\Omega \rightarrow -\Omega$, $i \rightarrow
180^\circ - i$ \citep[Section~5.2]{Skowron:2011}.  Because that sign is not
a goodness-of-fit question --- getting it backwards raises no $\chi^2$, it
silently reports the reflected orbit --- we pin it against a synthetic orbit
of known inclination rather than against any fit, and we validate the
Keplerian mode against first-principles synthetic orbits rather than against
another code's Keplerian implementation.\footnote{We record, since it is the
kind of thing this section exists to prevent, that \texttt{MulensModel}
3.11.0's Keplerian lens orbital motion contradicts its own linear mode on
this sign: with $\dot{\alpha} = +40^\circ\,{\rm yr}^{-1}$ its linear branch
returns $+40$ at $t_{0,\rm kep}$, as it must by definition, while its
Keplerian variants return $-40$.  The composition needs the minus of
Equation~(\ref{eq:gammaperp}).  The package's linear branch, its
$\gamma_\perp$ property and every static and parallax convention of C18 are
unaffected, and remain the references our parity tests use.}

Finally, and most portably: \emph{a published $(\Omega, i)$ pair is
meaningless without its frame's third axis}.  Reversing that axis flips the
handedness of the sky pair, sending $i \rightarrow 180^\circ - i$ and
reversing the sense in which $\Omega$ is measured, and a code reporting
$i > 90^\circ$ for a system another code reports at $i < 90^\circ$ is
usually announcing its third axis rather than disagreeing about the orbit.
\citet{Skowron:2011} Appendix~B is a case in point and a common one, since
it is the frame in which microlensing orbital solutions are most often
quoted: its first axis is the binary axis and its third points
\emph{toward} the observer, the opposite of C1's $+Z$.  \citet{Yee:2016}
quote the OGLE-2009-BLG-020L orbit in that frame, so their tabulated values
do not drop into the sky frame unchanged.  Both angles move:
\begin{equation}
  i = 180^\circ - i_{\rm B}, \qquad
  \Omega = \mathrm{PA_{axis}}(t_{0,\rm kep}) - \Omega_{\rm B},
  \label{eq:appb}
\end{equation}
the first because of the third axis and the second because $\Omega_{\rm B}$
is referred to the binary axis rather than to North.  Their $i_{\rm B} =
129.424^\circ$ is $i = 50.576^\circ$ here.  The node cannot be converted
from the published table alone, because $\mathrm{PA_{axis}}$ is not a
published quantity --- it is $\phi_\pi - \alpha_0$ by
Equation~(\ref{eq:alphakep}), and $\phi_\pi$ is an output of the fit --- so
we convert it against the built model rather than reading it off, and the
likelihood is sharp enough in it ($\pm 10^\circ$ costs more than
$10^6$ nats) that rounding the result is not an option.\footnote{A
transcription hazard in the same appendix, recorded for anyone working from
it: Equation~B9 displays its two rotation matrices under swapped labels ---
the matrix printed as $R_x$ rotates about the third axis and the one
printed as $R_z$ about the first --- while the surrounding text and every
subsequent use require the standard $z$--$x$--$z$ composition.  The paper's
own results are unaffected; only a literal transcription of B9 is.}
...
\paragraph{C29 --- a band extinction is not a number until the convention
is stated.}  A quoted $A_{\rm W149}$ cannot be compared to a modelled one
without saying how it was computed, because two inequivalent definitions are
in common use and for a filter as wide as Roman's W149 ($F146$, spanning
$0.896$--$2.073\,\mu$m with an effective width of $0.789\,\mu$m) they differ
by ${\sim}15\%$.  The \emph{monochromatic} definition evaluates the
reddening law at a single wavelength, $A_{\rm band} = A_V\,k(\lambda_{\rm
band})/k(V)$, and makes ratios between bands independent of $A_V$.  The
\emph{integrated} definition --- which is what a measurement actually is,
and what our bolometric-correction tables compute --- reddens the spectrum
and integrates it through the passband.  These are not the same quantity:
as $A_V$ rises the blue wing of a wide band is extinguished away, the band's
own effective wavelength drifts redward, and $dA_{\rm band}/dA_V$ therefore
\emph{falls}, from $0.272$ near $A_V = 0$ to $0.212$ at $A_V = 10$ for
$F146$.  Band ratios under the integrated definition are functions of $A_V$.

The consequence for the data challenge is concrete.  Its key is
monochromatic at each filter's effective wavelength: the ratio $A_{\rm
Z087}/A_{\rm W149} = 1.9236$ is reproduced to $0.1\%$ by the reddening law
evaluated at $0.86510$ and $1.30496\,\mu$m, and by no other published
wavelength definition for these filters (the pivot wavelength is $13.3\%$
away, the mean wavelength $24.7\%$), and the same holds across the sample
(median $1.9315$ over 293 events).  No single $A_V$ can therefore satisfy
both band extinctions in an integrated model, and the choice of which to
match is worth $15\%$: anchoring $A_{\rm W149}$ gives $A_V = 11.75$,
anchoring $A_{\rm Z087}$ gives $10.14$, and anchoring the colour excess
$E(Z087-W149)$ gives $9.01$.  We anchor the colour.  At $A_V = 9.01$ both
bands are reproduced $0.58$ mag faint --- the \emph{same} offset in each ---
and a common-mode grey offset is degenerate with distance and stellar
radius, which the spectral-energy-distribution fit already constrains; every
other anchor instead places the residual in colour, where only the fitted
effective temperature can absorb it ($+0.81$ mag at the $A_{\rm W149}$
anchor).  This is neither an error in the reddening law, which reproduces
the key's own ratio to $0.1\%$, nor in the tables: it is irreducible while
the data follow one definition and the model the other.  For real
observations the integrated treatment is the correct one and the ambiguity
does not arise.  We record the general caution because $F146$'s tabulated
central wavelengths alone span $1.305$--$1.538\,\mu$m, an $18\%$ range
before any physics, which maps to $24\%$ in $k/k_V$.
