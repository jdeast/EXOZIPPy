...
$u = \sqrt{2}$ and decaying only as $\theta_{\rm E}/u$ \citep{Dominik:2000}.
It has support for years on either side of a $t_{\rm E}$-long photometric
event and so belongs inside the full astrometric model rather than in an
event-window correction; $\theta_{\rm E}$ enters as a linear amplitude on a
shape the light curve already determines, which is what makes it a direct
measurement of the Einstein radius.  Measured from the \emph{lens} the same
