...
def write_param_file(
    ...
    with open(output_path, "w") as f:
        ...
        # The seed the fit actually ran with (run.py stamps it whether the
        # user named it or it was drawn), so the run is recoverable from its
        # own output -- the same reason this file carries start values at all.
        # It goes in a COMMENT, not as a key: this is a params file, and
        # `sampler:` belongs in the config, where a reader can paste it.
        _run_seed = getattr(idata, "posterior", None)
        _run_seed = (
            getattr(_run_seed, "attrs", {}).get("random_seed")
            if _run_seed is not None
            else None
        )
        if _run_seed is not None:
            f.write(
                f"# The fit this came from ran with sampler: "
                f"{{seed: {int(_run_seed)}}}  -- add that to the CONFIG "
                f"(not here) to\n"
                f"# reproduce it.  Bit-reproducible for nuts/numpyro/"
                f"blackjax/nutpie/ptde; ptde_async\n"
                f"# fixes the draws but not the arrival order (see "
                f"samplers/samplers.md).\n"
            )
        if K > 1:
            f.write(
