...
def get_ephemeris(target_id, start_time, stop_time, step="1h", outfile=None):
    ...
    header = (
        f"Target: {target_id} (Center: 500@0, frame: ICRF/J2000 equatorial)\n"
        f"Source: JPL Horizons\nColumns: BJD_TDB, X [AU], Y [AU], Z [AU]"
    )

    np.savetxt(
        outfile, data, header=header, fmt=["%.8f", "%.12f", "%.12f", "%.12f"]
    )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download Barycentric Ephemeris from JPL"
    )
    parser.add_argument(
        "--id",
        type=str,
        default="-79",
        help="Target ID (default: -79 for Spitzer)",
    )
    parser.add_argument(
        "--start", type=str, required=True, help="Start date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--stop", type=str, required=True, help="Stop date (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--step", type=str, default="1h", help="Step size (default: 1h)"
    )
